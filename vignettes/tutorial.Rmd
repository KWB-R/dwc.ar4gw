...
kwb.python::conda_py_install(env_name = "flopy", 
                             pkgs = list(conda = c("python", "flopy", "pyshp"), 
                                         py = NULL))
...
specific_discharge <- flopy$utils$postprocessing$get_specific_discharge(
  model = mf, 
  cbcfile = paths$mf_cbc,
  hdsfile = paths$binary_heads,
  kstpkper = c(0,0)
  )
# Does not work !!!!
# Fehler in py_call_impl(callable, dots$args, dots$keywords) : 
#  AttributeError: 'NoneType' object has no attribute 'laycon'
